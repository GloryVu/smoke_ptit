...
RUN apt-get -qq update \
    ...
    && pip install --default-timeout=1000 -r /requirements-ov.txt
...
# # Install required GPU-related libraries
...
# # Set an environment variable to specify the ONNX Runtime version to install
# ENV ONNXRUNTIME_VERSION=1.10.0
...
RUN pip3 wheel --default-timeout=100 --wheel-dir=/wheels -r requirements-wheels.txt
...
RUN export CUDA_PATH=/usr/local/cuda \
    && export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:~/TensorRT-TensorRT-8.6.1/lib
...
RUN --mount=type=bind,from=trt-wheels,source=/trt-wheels,target=/deps/trt-wheels \
    pip3 install /deps/trt-wheels/*.whl
