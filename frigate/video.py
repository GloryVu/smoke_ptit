...
def create_tensor_input(frame, model_config, region):
    ...
    # logger.info(f'{cropped_frame.shape}')
...
def detect(
    ...
    # cv2.imwrite(f'/media/frigate/clips/detect.jpg',np.squeeze(frame))
    tensor_input = create_tensor_input(frame, model_config, region)
    # cv2.imwrite(f'/media/frigate/clips/detect.jpg',np.squeeze(tensor_input))
    h =  frame.shape[0]*2//3
    w = frame.shape[1]
    ...
    for d in region_detections:
        ...
        x_min = int(max(0, (box[0] * w)))
        y_min = int(max(0, (box[1] * h)))
        x_max = int(min(w - 1, box[2] * w))
        y_max = int(min(h - 1,box[3] * h))
        # logger.info(f'{x_min},{y_min},{x_max},{y_max}')
        # logger.info(f'h: {detect_config.height}, w: {detect_config.width}')
...
def process_frames(
    ...
    while not stop_event.is_set():
        ...
        # frame = frame_manager.get(
        #     f"{camera_name}{frame_time}", (frame_shape[0], frame_shape[1])
        # )
        # cv2.imwrite(f'/media/frigate/clips/detect{c}.jpg',np.squeeze(frame))

        # logger.info(f'frame process {frame.shape}')
        if frame is None:
        ...
        if not detection_enabled.value:
            ...
            regions = [a
                ...
                # )
                for a in motion_boxes#combined_boxes
            ]
