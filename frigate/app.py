        ...
        for name in self.config.cameras.keys():
            ...
            try:
                shm_out = mp.shared_memory.SharedMemory(
                    name=f"out-{name}", create=True, size=20 * 8 * 4
                )
            except FileExistsError:
            ...
            try:
                ...
                largest_windowsize = max(
                    [
                        det.model.windowsize
                        for (name, det) in self.config.detectors.items()
                    ]
                )
                shm_out_q = mp.shared_memory.SharedMemory(
                    name=f"out_q-{name}", create=True, size=largest_windowsize*20 * 8 * 4
                )
            except FileExistsError:
                shm_out_q = mp.shared_memory.SharedMemory(name=f"out_q-{name}")
            try:
                # logger.info(f'{largest_frame}  {largest_windowsize}')
                shm_frame_q = mp.shared_memory.SharedMemory(
                    name=f"frame_q-{name}", create=True, size=largest_windowsize*largest_frame
                )
            except FileExistsError:
                shm_frame_q = mp.shared_memory.SharedMemory(name=f"frame_q-{name}")
            try:
                # logger.info(f'{largest_frame}  {largest_windowsize}')
                shm_segmentor = mp.shared_memory.SharedMemory(
                    name=f"segmentor-{name}", create=True, size=int(largest_frame/3)
                )
            except FileExistsError:
                shm_segmentor = mp.shared_memory.SharedMemory(name=f"segmentor-{name}")
            self.detection_shms.append(shm_in)
            self.detection_shms.append(shm_out)
            self.detection_shms.append(shm_out_q)
            self.detection_shms.append(shm_frame_q)
            self.detection_shms.append(shm_segmentor)
